...
res=convert(res)
data1=[]
with open('./temp/res'+args.num+'.txt', 'r') as f:   #改这里
    for line in f:
        data1.append(line.strip('\n'))
f.close()
new_data=[]
all_data1=convert(data1)
all_data1=list(set(all_data1)-set(res))
...
model = torch.load('./model/'+args.num+'.pth')   #改这里
...
torch.save(model, './model/'+args.num+'.pth')       #改这里